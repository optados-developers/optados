...
qe_bulk = re.compile("Bulk\ \s*([0-9\.-]+)\s*")
qe_total = re.compile("Total\ Quantum\ Efficiency\ \(electrons\/photon\)\:\s*([0-9\.-]+)\s*") 
mte = re.compile("Weighted\ Mean\ Transverse\ Energy\ \(eV\)\:\s*([0-9\.-]+)\s*")
...
def parse(fname):
    ...
    for lno, l in enumerate(lines):
        ...
        if match:
            retdict['qe_bulk'].append(float(match.groups()[0]))
            continue

        match = qe_total.search(l)
        if match:
            retdict['qe_total'].append(float(match.groups()[0]))
            continue

        match = mte.search(l)
        if match:
            retdict['mte'].append(float(match.groups()[0]))
            continue
